let $doc-name := xmldb:store("/db/mdrepo-data/", "stats_cmdi-providers.xml", <result></result>),
    ...
    $collection := collection("/db/mdrepo-data/cmdi-providers"),
$items := (<item key="C" label="CMD" >count($collection//CMD)</item> ,
<item key="C.-handle" label="CMD without MdSelfLink" >count($collection//CMD[not(Header/MdSelfLink)])</item>,
...
<item key="C.R.1" label="CMD with relative Resource (starts-with('.'))" >count($collection//CMD[.//ResourceType='Resource'][starts-with(.//normalize-space(ResourceRef),'.')])</item>,
<item key="C.R.2" label="CMD with http-uri Resource" >count($collection//CMD[.//ResourceType='Resource'][starts-with(.//normalize-space(ResourceRef),'http')])</item>,
<item key="C.R.1+2" label="CMD - with relative and http Resource">count($collection//CMD[.//ResourceType='Resource'][starts-with(.//ResourceRef,'.')][starts-with(.//normalize-space(ResourceRef),'http')])</item>,
...
<item key="C.cc.link-match" label="CMD with MdSelfLink=ResourceRef" >count($collection//MdSelfLink[. = $collection//ResourceProxy[ResourceType eq 'Metadata']/ResourceRef])</item>,
<item key="C.cc.orphans" label="CMD with orphaned MdSelfLink" >count($collection//MdSelfLink[not(. = $collection//ResourceProxy[ResourceType eq 'Metadata']/ResourceRef)])</item>,
<item key="C.cc.deadlinks" label="CMD with dead Metadata-ResourceRef" >count($collection//ResourceRef[not(. =  $collection//MdSelfLink)][../ResourceType eq 'Metadata'])</item>,

<item key="C.M" label="CMD with Metadata = Collections" >count($collection//CMD[.//ResourceType='Metadata'])</item>,
<item key="C.M.1" label="CMD with relative MD" >count($collection//CMD[.//ResourceType='Metadata'][starts-with(.//normalize-space(ResourceRef),'.')])</item>,
<item key="C.M.2" label="CMD with http-uri MD" >count($collection//CMD[.//ResourceType='Metadata'][starts-with(.//normalize-space(ResourceRef),'http')])</item>
)
let $upd-dummy :=   
        for $item in $items
           ...
           return update insert <item key="{$item/@key}" label="{$item/@label}" dur="{$duration}">{$answer}</item> into $doc/result

...
<xsl:variable name="dict" select="document($dict_file)" />
	...
	<xsl:template name="formURL" >		
		<xsl:param name="action" select="'search'"/>
		...
		<xsl:variable name="param_q">
					<xsl:if test="$q != ''">
							<xsl:value-of select="concat('&amp;query=',$q)"></xsl:value-of>
					</xsl:if>
		</xsl:variable>
		<xsl:variable name="param_repository">
					<xsl:if test="$repository != ''">
							<xsl:value-of select="concat('&amp;repository=',$repository)"></xsl:value-of>
					</xsl:if>
		</xsl:variable>
		<xsl:variable name="param_startRecord">
						<xsl:if test="$startRecord != ''">
							<xsl:value-of select="concat('&amp;startRecord=',$startRecord)"></xsl:value-of>
						</xsl:if>
		</xsl:variable>
		<xsl:variable name="param_maximumRecords">
						<xsl:if test="$maximumRecords != ''">
							<xsl:value-of select="concat('&amp;maximumRecords=',$maximumRecords)"></xsl:value-of>
						</xsl:if>
		</xsl:variable>		
		...
		<xsl:choose>
			...
			<xsl:otherwise>					
				...
				<xsl:otherwise>
					<xsl:value-of select="concat($base_dir, '/',$action, '/', $format, '?query=', $q, $param_repository, $param_startRecord, $param_maximumRecords)" />
				</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>

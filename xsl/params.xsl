...
<xsl:stylesheet
    ...
    <xd:doc>
        <xd:desc>A URL for locating JavsScript scripts and CSS style sheets
        <xd:p>
            Note: If this parameter is supplied it has to end in a path separator (/)!
        </xd:p>
        <xd:p>
            Note: These paths are meant for the clients browser so localhost is usually not an option!
        </xd:p>
        <xd:p>
